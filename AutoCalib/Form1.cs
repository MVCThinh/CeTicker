...
namespace AutoCalib
{
    public partial class Form1 : Form
    {
        ...
        private Dictionary<string, int> mapCardScore;
        ...
        public Form1()
        {
            ...
            mapCardScore = new Dictionary<string, int>();
            listCard = new List<string>(52 * (int)numudALlPoker.Value);
        }

        public void CreateCard(Dictionary<string, int> mapCardScore, List<string> CardArray)
        {
            List<string> soBoBai = new List<string>();
            for (int i = 0; i < (int)numudALlPoker.Value; i++)
            {
                soBoBai.Add("Bộ " + (i + 1));
            }
        ...
        {
            CreateCard(mapCardScore, listCard);
            if (mapCardScore.Count == listCard.Count)
            {
                lbSuccess.Text = "Tạo thành công";
                lbSuccess.BackColor = Color.Green;
                lbAllCards.Text = mapCardScore.Count.ToString();
            }
            else
            {
                lbSuccess.Text = "Tạo thất bại";
                lbSuccess.BackColor = Color.Red;
                lbAllCards.Text = "0";
            }
        }
    }
